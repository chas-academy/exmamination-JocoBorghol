...
function add(type) {          
  ...
  balOut.textContent = balance;  //gör om saldot (2) med 2 decimaler,textcontent raden skriver ut det uppdaterade saldot


  const li = document.createElement("li");  //skapa och spara en rad i listan
  li.innerHTML =
      `<span>${text}</span><span>${type === "inc" ? "+" : "−"}${val} kr</span>`; //fyller listan med + på inkomst och - på utgifter
